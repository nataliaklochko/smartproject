...
f = FeatureExtractor(model=VGGBottleneckModel, imageset_dir="image_dataset", types=types)
...
def train_triplet_net():
    ...
    with open("model_{}_freezed.json".format(num_to_freeze), "w") as json_file:
        json_file.write(model_json)

    model = train(model, generator=f.triplet_batch_generator(), num_epochs=num_epochs)

    model_json = model.to_json()
    json_file = open("vgg_triplet_net_{0}.json".format(num_epochs), "w")
    json_file.write(model_json)
    json_file.close()
    model.save_weights("vgg_triplet_net_{0}.h5".format(num_epochs))
